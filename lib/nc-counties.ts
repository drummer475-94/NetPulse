export const ncCountyIdentities = [
...
].map((name, index) => ({ name, fips: `37${String(index * 2 + 1).padStart(3, "0")}` }));

export const ncCountyByName = new Map(
  ncCountyIdentities.map((county) => [county.name.toUpperCase(), county]),
);
